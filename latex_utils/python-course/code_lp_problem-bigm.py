...
#    lp_dict = json.loads(l)
    ...
    if True:
        pulp_problem = lp.as_pulp_problem()
        print(type(pulp_problem))
        pulp_problem.solve()
        from pulp import *
        print(isinstance(pulp_problem, pulp.LpProblem))
        #print(pulp_problem)
        #print(pulp.LpStatus[pulp_problem.status])
