...
\jie 需明确此问题为产销不平衡运输问题。引入虚拟销地$B4$，将问题标准化为
...
A3&8&7&9&0&90\\ \hline
需求量&60&100&50&30\\
\hline\hline\end{mytabular}\end{keytable}

%===========答案
\def\ori{  {  {8,7,9,0},{2,3,8,0},{4,6,5,0}  }  }
\def\sremainder{
\draw (sr0) node{\footnotesize 90};
\draw (sr1) node{\footnotesize 80};
\draw (sr2) node{\footnotesize 70};
}
\def\dremainder{
\draw (dr0) node{\footnotesize 60};
\draw (dr1) node{\footnotesize 100};
\draw (dr2) node{\footnotesize 50};
\draw (dr3) node{\footnotesize 30};
}
\def\total{\footnotesize 240}

用最小元素法得到一个初始基本可行方案。

由于单位运费为0的单元格有多个，所以本问题用最小元素法有多种可能性，
以下为其中一种。不同的初始方案，迭代次数不同，以下这个方案只需1次优化就能迭代出最优方案。

说明：对于最小元素法，并没有哪种套路保证得到的初始基本可行方案可以减少迭代的次数。
...
所有非基变量格检验数大于0，当前方案为{唯一}最优方案，总运费为950。
