...
#    r.clipboard_clear ()
    ...
    if "NCM" in used_method_list:
        NCM_result = t.solve(init_method="NCM")
        if not NCM_result.has_degenerated_init_solution:
            continue
        count1 += 1
        method_result_list.append(NCM_result)

    template = latex_jinja_env.get_template('/utils/final_test_transportation.tex')
    ...
    tex = template.render(
        ...
        used_method_list=[INIT_METHOD_DICT.get(used_method, ["NCM"]) for used_method in required_init_method],
    )
