...
<p align="middle"><strong>答案表\VAR{answer_table_iters.next()}</strong> \BLOCK{ if lp2phase.solutionCommon.need_two_phase }第一阶段\BLOCK{ endif }</br>
{% call latex(compiler="pdflatex", image_format="png", tex_preamble=preamble)  %}
...
<p align="middle"><strong>答案表\VAR{answer_table_iters.next()}</strong> \BLOCK{ if lp2phase.solutionPhase1.need_two_phase }第二阶段\BLOCK{ endif }</br>
{% call latex(compiler="pdflatex", image_format="png", tex_preamble=preamble)  %}
\begin{keytable}[H]\centering
