...
\BLOCK{ if lp.solutionCommon.artificial_str_list_intro and lp.solutionCommon.method == "modified_simplex" and show_big_m }
...
\BLOCK{ set answer_table_counter=answer_table_counter+1 }
<p align="middle"><strong>答案表\VAR{answer_table_counter}</strong> \BLOCK{ if lp2phase.solutionCommon.need_two_phase }第一阶段\BLOCK{ endif }</br>
{% call latex(compiler="pdflatex", image_format="png", tex_preamble=preamble)  %}
...
\BLOCK{ endif }
\BLOCK{ set answer_table_counter=answer_table_counter+1 }
<p align="middle"><strong>答案表\VAR{answer_table_counter}</strong> \BLOCK{ if lp2phase.solutionPhase1.need_two_phase }第二阶段\BLOCK{ endif }</br>
{% call latex(compiler="pdflatex", image_format="png", tex_preamble=preamble)  %}
\begin{keytable}[H]\centering
...
$x_{\VAR{lp.solutionPhase2.pivot_list[idx][1]+1}}$为入基变量。计算$\bar p_{\VAR{lp.solutionPhase2.pivot_list[idx][1]+1}}$和$\bar b$以确定出基变量：
\BLOCK{ endif }
\BLOCK{ if not loop.last }$$\mathbf{\bar p}_{\VAR{lp.solutionPhase2.pivot_list[idx][1]+1}}=\mathbf{B}^{-1}{\mathbf{p}_{\VAR{lp.solutionPhase2.pivot_list[idx][0]+1}}}=\begin{bmatrix}
