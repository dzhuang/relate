...
<p align="middle"><strong>表\VAR{question_iters.next()}</strong></br>
...
<p align="middle"><strong>答案表\VAR{iters.next()}</strong></br>
{% call latex(compiler="pdflatex", image_format="png", tex_preamble=preamble)  %}
...
\BLOCK{ if verify }
<td> <p>检验数表为：</p>
<p align="middle"><strong>答案表\VAR{iters.next()} </strong> 检验数表</br>
{% call latex(compiler="pdflatex", image_format="png", tex_preamble=preamble, tex_preamble_extra=transport_preamble_extra)  %}
