...
\BLOCK{ for result in  lp.sa_result[:3]}
\BLOCK{ set outer_loop = loop }


\noindent {\hei (\VAR{loop.index}).\VAR{result.description}}
...
\BLOCK{ if outer_loop.first }（注：最优表中的$\mathbf{B^{-1}}=\VAR{result.B_1}$，下同.）\BLOCK{ endif }
