...
class lpSolver(object):
    ...
    def create_tableau(self):
        ...
        if n_T_extra_lines == 2:
            # Insert objective into tableau, 2 for 2 stage simplex method
...
class lpBigMSolver(lpSolver):
    ...
    def set_up_tableau(self):
        ...
        # 大M法的目标函数Get the goal of the big-m problem
...
def linprog(c, A_ub=None, b_ub=None, A_eq=None, b_eq=None,
    ...
    if meth == 'simplex':
        ...
        #                      bounds=bounds, callback=callback, cnstr_orig_order=cnstr_orig_order, **options)
        # return lp.solve()
        return _linprog_big_m_simplex(c, A_ub=A_ub, b_ub=b_ub, A_eq=A_eq, b_eq=b_eq,
                                       bounds=bounds, callback=callback, cnstr_orig_order=cnstr_orig_order, **options)
    else:
...
def _linprog_big_m_simplex(c, A_ub=None, b_ub=None, A_eq=None, b_eq=None,
    ...
    # Create the tableau
    T = np.zeros([m + 1, n + n_slack + n_artificial + 1])
    ...
    return OptimizeResult(x=x, fun=obj, nit=int(nit), status=status, slack=slack,
                           slack_list=slack_list, artificial_list=artificial_list, init_tablaeu=init_tablaeu,
                           message=messages[status], success=(status == 0))
