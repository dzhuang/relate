...
\BLOCK{ if lpBigM.need_artificial_variable }
...
\topcaption{}
\begin{tabular}{c|c|\BLOCK{ for x in range(0, lpBigM.solutionPhase2.all_variable_str_list|length) }
\BLOCK{ if lpBigM.need_artificial_variable }C{1.5cm}\BLOCK{ else }C{1cm}\BLOCK{ endif }
...
\BLOCK{ else }
该问题\VAR{lpBigM.solve_status_message}
\BLOCK{ endif }
\BLOCK{- if lpBigM.solve_opt_res_str -}，\VAR{lpBigM.solve_opt_res_str}\BLOCK{ endif -}.

\BLOCK{ endif -} \#{ show_big_m }

\BLOCK{ if show_2_stage }

\BLOCK{ if show_big_m and lpBigM }
<hr>
\BLOCK{ endif -}

\BLOCK{ if lp.solutionCommon.artificial_str_list_intro }<strong>两阶段法</strong>：引入人工变量$\VAR{lp.solutionCommon.artificial_str_list_intro|join(",\,") }$，构造辅助问题：\BLOCK{ endif -}
...
\BLOCK{ if not lp.solutionCommon.need_two_phase }用\BLOCK{if lp.solutionPhase2.method=="dual_simplex" }对偶\BLOCK{ endif }单纯形法求解：\BLOCK{ else }第一阶段，求解辅助问题：\BLOCK{ endif -}
