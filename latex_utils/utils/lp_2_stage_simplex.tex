...
$$\mathbf{B}=(\VAR{lp.solutionPhase2.modi_bp_list[idx]|join(", ")})=\begin{pmatrix}
...
\BLOCK{ endif }
\BLOCK{ if not loop.last }$$\mathbf{\bar p}_{\VAR{lp.solutionPhase2.pivot_list[idx][0]+1}}=\mathbf{B}^{-1}{\mathbf{p}_{\VAR{lp.solutionPhase2.pivot_list[idx][0]+1}}}=\begin{pmatrix}
\VAR{lp.solutionPhase2.modi_pj_list[idx]|join(" \\\\ ")}
\end{pmatrix},
\quad
\mathbf{\bar b}=\mathbf{B}^{-1}\mathbf{b}=\begin{pmatrix} \VAR{lp.solutionPhase2.modi_b_list[idx]|join(" \\\\ ")} \end{pmatrix}$$
...
该问题\VAR{lp.solve_status_message}
\BLOCK{ endif }
$$\mathbf{X^*_B}= \mathbf{\bar b}=\mathbf{B}^{-1}\mathbf{b}=\begin{pmatrix} \VAR{lp.solutionPhase2.modi_b_list[idx]|join(" \\\\ ")} \end{pmatrix}$$
