...
for l in lp_json_list_loaded:
    ...
    # --------------------------------------------------选择迭代次数
    if not lp.solutionCommon.nit in [4]:
        continue
    lp.sensitive_analysis()

    template = latex_jinja_env.get_template('/utils/final_test_lp_sensitivity.tex')
    tex = template.render(
...
#        standardized_lp = lp.standardized_LP(),
    ...
    if lp.solutionCommon.nit in [2, 3] and lp.res.status == 0:
        r.clipboard_append(tex)
        final_lp_list1.append(lp.json)
        count1 += 1
    if lp.solutionCommon.nit in [4] and lp.res.status == 0:
