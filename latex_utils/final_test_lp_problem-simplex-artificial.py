...
#    lp_dict = json.loads(l)
    ...
    if len(lp.solutionCommon.artificial_str_list_intro) !=2:
        continue

    # a=lp.standardized_LP()
    # print(a.get_goal_str(purelatex=True))


    template = latex_jinja_env.get_template('/utils/final_test_artificial_blank.tex')
    tex = template.render(
        ...
        pre_description1=u"用大M法求解线性规划问题",
        pre_description2=u"在用两阶段法求解线性规划问题",
        after_description1=r"时，需要(完成标准化后)分别在式(1)和(2)中引入人工变量(设为$%s$)，"
                           r"且需要将目标函数改写为："% ",\, ".join(lp.solutionCommon.artificial_str_list_intro),
        after_description2=r"的第一阶段，需要(完成标准化后)分别在式(1)和(2)中引入人工变量(设为$%s$)，"
                           r"则第一阶段所构造辅助问题的目标函数为：" % ",\, ".join(lp.solutionCommon.artificial_str_list_intro),
