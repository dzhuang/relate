...
TEST_CONFIG_NO_DOCKER_IMAGE = deepcopy(TEST_DOCKERS_CONFIG)
TEST_CONFIG_NO_DOCKER_IMAGE.pop("docker_image")

TEST_CONFIG_DOCKER_MACHINE_NOT_ENABLED = deepcopy(TEST_DOCKERS_CONFIG)
...
class GetDockerClientConfigTests(SimpleTestCase):
    ...
    @override_settings(RELATE_RUNPY_DOCKER_CONFIG=TEST_DOCKERS_CONFIG)
    def test_runpy_client_docker_machine_conf(self, mocked_register, mocked_sys):
        result = get_docker_client_config(TEST_DOCKERS_CONFIG, for_runpy=True)
        self.assertIsInstance(result, RunpyClientForDockerMachineConfigure)
        self.assertEqual(result.image, TEST_DOCKERS_CONFIG["docker_image"])

    @mock.patch(
        "relate.utils.is_windows_platform", return_value=True)
    @override_settings(RELATE_RUNPY_DOCKER_CONFIG=TEST_DOCKERS_CONFIG)
    def test_client_docker_machine_conf(self, mocked_register, mocked_sys):
        result = get_docker_client_config(TEST_DOCKERS_CONFIG, for_runpy=False)
    ...
    # @mock.patch(
    #     "relate.utils.is_windows_platform", return_value=True)
    # @override_settings(RELATE_DOCKERS_CONFIG=TEST_DOCKERS_CONFIG,
    ...
    #         get_docker_client_config(TEST_DOCKERS_CONFIG, for_runpy=True)

    # @override_settings(RELATE_DOCKERS_CONFIG=TEST_DOCKER_CONFIG,
