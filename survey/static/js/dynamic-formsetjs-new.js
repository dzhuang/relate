(function () {
    ...
    //  mutually exclusive checkbox utility
    var exclusive_array = [];
    exclusive_array.push("fullcorrect", "完全正确", "未回答(基本未回答)");
    var null_answer_array = [];
    null_answer_array.push("--", "---", "----");
    function get_checkbox_html(ele) {
        return $(ele).siblings("label").html();
    }
    function siblings_with_same_name(ele){
        return $(ele).parent().siblings().find('[name=' + $(ele).attr('name') + ']');
    }
    function is_exclusive_checkbox(ele){
        return jQuery.inArray(get_checkbox_html($(ele)), exclusive_array) != -1
    }
    function is_null_answer_checkbox(ele){
        return jQuery.inArray(get_checkbox_html($(ele)), null_answer_array) != -1
    }
    function ensure_null_answer_checkbox_status(ele){
        // ensure if null answer check box is checked only and should
        // be checked when no other checkbox is checked.
        var numberOfChecked = $('input:checkbox:checked[name=' + $(ele).attr('name') + ']').length;
        if (numberOfChecked === 0){
            $(ele).parent().siblings()
                .andSelf() // andSelf is important
                .find('[name=' + $(ele).attr('name') + ']').each(function () {
