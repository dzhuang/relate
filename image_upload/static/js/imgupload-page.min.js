var EditModalSelector="#editModal",EditModalImgSelector="#image",cropperResultMessageBoxSelector="#cropper-message",cropperControlButtonDivSelector=".cropper-btns",cropperControlStatusBtnSelector=".cropper-status-btns .btn";cropperControlRotateBtnSelector=".cropper-rotate-btns .btn";
(function(a,d,e,f){a.widget("blueimp.fileupload",a.blueimp.fileupload,{options:{editModalId:EditModalSelector,editModalImgId:EditModalImgSelector,cropperResultMessageBoxSelector:cropperResultMessageBoxSelector,cropperControlButtonDivSelector:cropperControlButtonDivSelector,added:function(b, c){var h=a(this),h=h.data("blueimp-fileupload")||h.data("fileupload"),d=h.options;c.replaceChild&&c.replaceChild.length&&(a(d.editModalId),a(c.context).replaceAll(c.replaceChild),h._trigger("replaced",b,c));
...
var c=a(b.currentTarget),d=this.options,e=this.element,f=c.closest(".template-upload,.template-download");b=f.hasClass("template-download")?"download":"upload";var k=a(d.editModalId),g=a(d.editModalImgId);if("upload"===b){var l=f.data("data");if(!l){c.prop("disabled",!0);return}var m,n=l.files[0];a.each(l.originalFiles,function(a, c){if(c.name===n.name)return m=c,!0});if(!m)return;g.prop("src",loadImage.createObjectURL(m));g.processCroppedCanvas=function(c){k.modal("hide");a(d.cropperControlButtonDivSelector).find(".btn").prop("disabled",
!0);f.find(".btn").prop("disabled",!0);c.toBlob(function(c){c.name=n.name;d.maxNumberOfFiles++;e.fileupload("add",{files:[c],replaceChild:a(l.context)});d.maxNumberOfFiles--},m.type)}}"download"===b&&(g.attr("src",c.data("src")),g.submitData=function(b){var f=a(d.cropperResultMessageBoxSelector);a(d.cropperControlButtonDivSelector).find(".btn").prop("disabled",!0);g.cropper("disable");a.ajax({method:"POST",url:c.data("action"),data:JSON.stringify(b),beforeSend:function(a,b){a.setRequestHeader("X-CSRFToken",
...
f=b.height/e.height*e.width;f>=b.width?(e=b.width/e.width*e.height,b={height:e,width:b.width,top:(b.height-e)/2,left:0}):b={height:b.height,width:f,top:0,left:(b.width-f)/2};c.cropper("setCanvasData",b).cropper("setCropBoxData",b);a(d.cropperControlButtonDivSelector).find(".btn").prop("disabled",!1)};k.modal("show",[g,b])}})})(jQuery,window,document);
$(function(){var a,d,e=$(EditModalSelector),f;e.on("show.bs.modal",function(b){a=b.relatedTarget[0];d=b.relatedTarget[1]}).on("shown.bs.modal",function(){$(this).data("new",!0);$(".relate-save-button").addClass("disabled");a.cropper({viewMode:1,checkOrientation:!1,autoCrop:!0,autoCropArea:1,strict:!0,movable:!1,zoomable:!1,minContainerheight:.8*$(window).height(),ready:function(b){f=a.cropper("getData","true");e.find(cropperControlRotateBtnSelector).prop("disabled",!1)},cropstart:function(a){},crop:function(a){},
cropend:function(b){b=a.cropper("getData","true");b=JSON.stringify(f)===JSON.stringify(b);e.find(cropperControlStatusBtnSelector).prop("disabled",b)}})}).on("hidden.bs.modal",function(){$(".relate-save-button").removeClass("disabled");a.cropper("destroy");$(this).attr("data-new",!1).find(cropperControlButtonDivSelector).find(".btn").prop("disabled",!0).end().end().find(cropperResultMessageBoxSelector).empty().end().active=!1}).on("show.bs.modal",function(a){getWidthQueryMatch_md_sm_xs()&&window.history.pushState("forward",
null,"#edit")}).on("hide.bs.modal",function(a){"#edit"===location.hash&&getWidthQueryMatch_md_sm_xs()&&window.history.back()});e.find(cropperControlButtonDivSelector).on("click","[data-method]",function(){var b=$.extend({},$(this).data());if("rotate"===b.method){var c=a.cropper("getContainerData");a.cropper("setCropBoxData",{width:2,height:2,top:c.height/2-1,left:c.width/2-1})}else"commit"===b.method&&("download"===d?(b.method="getData",b.option="true"):"upload"===d&&(b.method="getCroppedCanvas"));c=
a.cropper(b.method,b.option,b.secondOption);switch(b.method){case "scaleX":case "scaleY":$(this).data("option",-b.option);break;case "reset":e.find(cropperControlStatusBtnSelector).prop("disabled",!0);break;case "getData":c&&a.submitData&&a.submitData(c);break;case "getCroppedCanvas":c&&a.processCroppedCanvas&&a.processCroppedCanvas(c);break;case "rotate":c&&a.rotateCanvas&&a.rotateCanvas()}b=a.cropper("getData","true");JSON.stringify(f)===JSON.stringify(b)&&e.find(cropperControlStatusBtnSelector).prop("disabled",
...
function hide_fileupload_sortable_handle(a){1<$(a).fileupload("option","filesContainer").children().length?$(a).find(".imageSortableHandle").removeClass("hidden"):$(a).find(".imageSortableHandle").addClass("hidden")}
